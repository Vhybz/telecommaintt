...
class AuthRepository {
  ...
  Future<List<Map<String, dynamic>>> getTechnicians() async {
    final response = await _client.from('profiles').select('id, full_name, phone').eq('role_id', 3).order('full_name');
    return List<Map<String, dynamic>>.from(response);
  }

  Future<List<Map<String, dynamic>>> getAllUsers() async {
    final response = await _client.from('profiles').select('id, full_name, phone').order('full_name');
    return List<Map<String, dynamic>>.from(response);
  }
