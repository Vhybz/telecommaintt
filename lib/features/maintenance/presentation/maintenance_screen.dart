...
class _MaintenanceScreenState extends ConsumerState<MaintenanceScreen> with SingleTickerProviderStateMixin {
  ...
  Widget _buildTaskCard(Map<String, dynamic> task) {
    ...
    final dateStr = task['scheduled_date'] != null 
        ? DateFormat('MMM dd, yyyy HH:mm').format(DateTime.parse(task['scheduled_date']))
        : 'Not Set';
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          ...
                          Text('Site: ${_getStationName(task)}', style: const TextStyle(color: AppColors.textSecondary, fontSize: 12)),
                          const SizedBox(width: 16),
              ...
              children: [
                Row(
                  ...
                  children: [
                    ...
                    Text('Tech: ${_getTechnicianName(task)}', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w500)),
                  ],
                ),
                Row(
                  mainAxisSize: MainAxisSize.min,
  ...
  String _getStationName(Map<String, dynamic> task) {
    final data = task['base_stations'];
  ...
  String _getTechnicianName(Map<String, dynamic> task) {
    // Check manual entry first
    if (task['manual_assigned_name'] != null && task['manual_assigned_name'].toString().isNotEmpty) {
      return task['manual_assigned_name'];
    }
    
    final data = task['technician'];
    if (data == null) return 'Unassigned';
    if (data is List) {
      if (data.isEmpty) return 'Unassigned';
      return data[0]['full_name'] ?? 'Unassigned';
    }
